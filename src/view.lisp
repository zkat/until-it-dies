...
(defreply make ((view =view=) &key (x 0) (y 0) (width 1) (height 1) (far 0) (near 10))
  (defobject =view= ((view-left x) (view-right (+ x width))
                     (view-bottom y) (view-top (+ y height))
                     (view-far far) (view-near near))))

(defun make-view (x y width height &key (far 0) (near 10) (prototype =view=))
  (make prototype :x x :y y :width width :height height :far far :near near))
...
(defreply set-view ((view =view=))
  (with-properties (view-left view-right view-top view-bottom view-far view-near)
    ...
    (gl:load-identity)
    (let* ((x-scale (if (> view-left view-right) -1 1))
           (x-translate (if (= x-scale 1) 0 (- (- view-left view-right))))
           (y-scale (if (> view-bottom view-top) -1 1))
           (y-translate (if (= y-scale 1) 0 (- (- view-bottom view-top))))
           (z-scale (if (> view-far view-near) -1 1))
           (z-translate (if (= z-scale 1) 0 (- (- view-far view-near)))))
